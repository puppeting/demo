...
import com.inclusive.finance.ui.ApplyActivity
import com.inclusive.finance.ui.ApplyActivity2
import com.inclusive.finance.ui.MainActivity
import razerdp.basepopup.BasePopupWindow
import razerdp.util.animation.AlphaConfig
import razerdp.util.animation.AnimationHelper

class AccountPop3(context: ApplyActivity2) : BasePopupWindow(context) {

    override fun onCreateContentView(): View {
        return createPopupById(R.layout.pop_account)
    }

    private var viewModel: ApplyModel

    init {
        setOutSideDismiss(false)
        RxBus.get().register(this)
        viewModel = ViewModelProvider(context).get(ApplyModel::class.java)
